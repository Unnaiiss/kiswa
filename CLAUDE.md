...
products/{id}: name, slug, description, notes (fragrance notes), priceInr, mrpInr, sizeMl, category, imageUrls[], isActive, stock (number), lowStockThreshold, createdAt. The stock field is the live stock — but it may ONLY be changed inside the recordSale/stockIn/stockAdjust transactions below, never edited directly.
stockMovements/{id}: productId, productName, qtyChange (+/-), reason ('opening_stock' | 'purchase' | 'online_sale' | 'offline_sale' | 'adjustment' | 'return'), referenceId (saleId if any), note, createdAt. Every stock change writes a movement — this is the audit trail.
sales/{id}: channel ('online' | 'offline'), invoiceNo (unique, 'KSW-YYYY-NNNN'), customerName, customerPhone, items[] (embedded array: productId, productName, unitPrice, qty, lineTotal), subtotal, discount, total, paymentMethod ('razorpay' | 'cash' | 'upi' | 'card'), paymentStatus, razorpayOrderId, razorpayPaymentId, orderStatus ('pending' | 'paid' | 'packed' | 'shipped' | 'delivered' | 'cancelled'), shippingAddress (map, online only), createdByUid, createdAt.
...
recordSale(...) lives in server code (lib/) using firebase-admin and a Firestore transaction: read all product docs and the invoice counter first, verify stock >= qty for every item (throw if not — overselling must be impossible), then decrement each product's stock, write one stockMovement per item, increment the invoice counter, and create the sale doc. All in ONE transaction (all reads before writes, per Firestore rules).
